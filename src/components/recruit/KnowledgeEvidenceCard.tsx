...
const verificationLabel = (card: EvidenceCard) => {
  if (card.verificationStatus === "verified") return "Verified source evidence";
  if (card.verificationStatus === "inference" || card.basis === "inference") return "Inference";
  return "Reference could not be verified";
};
  ...
  return (
    <article className="rounded-xl border border-uq bg-uq-glass-subtle p-3 text-xs">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="font-mono text-[9px] uppercase tracking-[0.1em] text-uq-3">Confidence: {card.confidence}</span>
      </div>
      ...
      {card.verificationNote && <p className="mt-1 text-[11px] text-uq-3">{card.verificationNote}</p>}
